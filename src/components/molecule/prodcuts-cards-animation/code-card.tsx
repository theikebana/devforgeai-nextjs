...
import { Cloud, Building2, ShieldCheck, Settings2, ScanSearch } from "lucide-react";
...
const Skeleton = () => {
  ...
  useEffect(() => {
    ...
    animate(sequence, {
      // @ts-expect-error - motion animate accepts repeat/repeatDelay
      repeat: Infinity,
      repeatDelay: 1,
    });
    // scale/transform are stable arrays; listing them would recreate sequence every render
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  return (
    <div className="p overflow-hidden h-full relative flex items-center justify-center">
  ...
  <Container className="h-12 w-12 circle-2">
    <Building2 className="h-6 w-6 dark:text-white" />
  </Container>
  ...
  <Container className="h-8 w-8 circle-5">
    <ScanSearch className="h-4 w-4" />
      ...
      <div className="h-40 w-px absolute top-0 m-auto z-40 bg-gradient-to-b from-transparent via-cyan-500 to-transparent animate-move">
        <div className="w-12 h-12 top-1/2 -translate-y-1/2 absolute -left-10">
          <Sparkles />
        </div>
      </div>
    </div>
...
const Sparkles = () => {
  // Deterministic "random" values per instance from useId (no Math.random during render)
  const id = React.useId();
  const values = React.useMemo(() => {
    const hash = (s: string) => {
      let n = 0;
      for (let i = 0; i < s.length; i++) n = (n * 31 + s.charCodeAt(i)) % 0x7fff;
      return Math.abs(n) / 0x7fff;
    };
    ...
    <div className="absolute inset-0">
      {values.map((v, i) => (
        <motion.span
          key={`star-${i}`}
          animate={{
            top: `calc(${v.r1 * 100}% + ${v.move}px)`,
            left: `calc(${v.r2 * 100}% + ${v.move}px)`,
            opacity: v.opacity,
            scale: [1, 1.2, 0],
          }}
          transition={{
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "max-w-xs w-full mx-auto  rounded-xl border border-[rgba(255,255,255,0.10)] dark:bg-[rgba(40,40,40,0.70)] bg-black/30 group",
        className
      )}
    >
      {children}
...
export const CardSkeletonContainer = ({
  ...
  return (
    <div
      className={cn(
        "h-[12rem]  rounded-xl z-40",
        className,
        showGradient &&
          "bg-black/2 dark:bg-[rgba(40,40,40,0.70)] "
      )}
    >
      {children}
    </div>
