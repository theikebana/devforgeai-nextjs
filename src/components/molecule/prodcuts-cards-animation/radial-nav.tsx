...
function useShortestRotation(target: number) {
  const [angle, setAngle] = React.useState(() => normalizeDeg(target));
...
function MenuButton({
  ...
  return (
    <motion.button
      ...
      className="relative flex space-x-1 items-center cursor-pointer overflow-hidden whitespace-nowrap glass-card !backdrop-blur-xl bg-black/30 rounded-full border border-white/10 text-foreground font-medium"
      style={{
        height: buttonSize,
        minWidth: buttonSize,
...
function RadialNav({
  ...
  return (
    <div
      className="relative flex items-center justify-center rounded-full border-2 mt-5 border-white/10 "
